...
class InspectionRepository extends BaseRepository<Inspection> {
  ...
  InspectionRepository({required this.box, required this.slug});
  ...
  Future<Inspection?> fetchFromApi() async {
    ...
    try {
      ...
      if (r.data.isNotEmpty) {
        inspection = Inspection.set(r.data);
      }
  ...
  Future<Inspection> update(Inspection inspection) async {
    ...
    n.setBody = {
      ...
      // 'rejected_notes': inspection.rejectedNote,
    };

    try {
      CustomResponse r = await n.response(RoutingUrl.inspection);
