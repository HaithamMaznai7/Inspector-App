...
class InspectionDetailsController extends GetxController
  ...
  /// Called by [VehicleDetailsController] after a successful save so the
  /// details header and home listing update without a pull-to-refresh.
  Future<void> onVehicleSaved(VehicleDetails v) async {
    vehicleDetails.value = v;
    await _refreshInspectionAfterVehicleEdit();
  }

  Future<void> _refreshInspectionAfterVehicleEdit() async {
    if (slug == null || repository == null) return;
    if (!ConnectionService.instance.isConnectionGood.value) return;
    try {
      final refreshed = await repository!.fetchFromApi();
      if (refreshed != null) {
        inspection.value = refreshed;
        // Notify the home listing so its card matches. Only attempts the find
        // — InspectionsController may not be registered if this detail was
        // opened via a deep link / notification.
        if (Get.isRegistered<InspectionsController>()) {
          Get.find<InspectionsController>().patchInspection(refreshed);
        }
      }
      update();
    } catch (e) {
      AppLogger.log('refreshInspectionAfterVehicleEdit:', '$e');
    }
  }
