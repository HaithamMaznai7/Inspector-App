...
import 'package:fahis_inspector/services/authentication/enums/verification_type.dart';
import 'package:fahis_inspector/services/authentication/widgets/otp_dialog.dart';
...
class CanVerification {
    ...
    try {
      switch (type) {
        case UserVerifiationType.mobile:
          url = await Auth.authRepo?.sentOTP(value);
        default:
          url = await Auth.authRepo?.sentOTP(value);
      }
  ...
  Future<void> _verifyOtp(String url, String code) async {
    ...
    try {
      await Auth.authRepo?.verifyOTP(url, code);
    } on FNetworkException catch (e) {
      e.notify();
    } catch (e) {
      FLoader.errorSnackBar(
